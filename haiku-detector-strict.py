...
def count_syllables(word):
    ...
    phones = dic.get(word, [[]])[0] # uses the first pronunciation from its list
    if phones:
...
def haiku_format(message):
    if count_sylls_text(message) == 17:
...
async def haiku_handler(room, message):
    ...
    if match.is_not_from_this_bot() and count_sylls_text(match.event.body) == 17:
